...
Messages are encrypted in the browser with AES-GCM before upload. A shared recovery secret derives a nonextractable browser key saved in IndexedDB. Save the recovery secret in a password manager before starting. New devices require that secret; device approval is not implemented. An unlocked owner can use Replace recovery key to re-encrypt history after saving a new recovery secret. Replacement preserves message IDs and delivery records, commits atomically, and rejects concurrent history changes. The browser saves the candidate key before committing so it can recover after a lost response. The operation supports up to 1,000 messages and 8 MB; larger histories are refused without changes. Existing copies cannot be revoked. Losing both the secret and unlocked devices makes old encrypted history unrecoverable. Sign-in does not recover encryption keys.
